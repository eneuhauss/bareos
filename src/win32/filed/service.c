...
int bareosServiceMain()
{
   ...
   if (have_service_api) {            /* New style service API */
      ...
      if (RegisterService == NULL) {
         MessageBox(NULL, _("Registry service not found: Bareos service not started"),
            APP_DESC, MB_OK);
...
int installService(const char *cmdOpts)
{
   ...
   if (have_service_api) {
      ...
      if (!bareosService) {
         ...
         MessageBox(NULL, _("The Bareos service: " APP_NAME " could not be installed"),
              APP_DESC, MB_ICONEXCLAMATION | MB_OK);
         return 0;
      }

      /* Set a text description in the service manager's control panel */
      set_service_description(serviceManager, bareosService,
(char *)_("Provides file backup and restore services. BAREOS -- Backup Archiving REcovery Open Sourced"));
      ...
      if (RegCreateKey(HKEY_LOCAL_MACHINE,
         ...
         MessageBox(NULL, _("The System Registry could not be updated - the Bareos service was not installed"),
             APP_DESC, MB_ICONEXCLAMATION | MB_OK);
         return 0;
      }
...
int removeService()
{
   if (have_service_api) {      /* Newer Windows platform (NT, Win2K, ...) */
      ...
      if (serviceManager) {
         ...
         if (bareosService) {
            ...
            if (ControlService(bareosService, SERVICE_CONTROL_STOP, &status)) {
               ...
               if (status.dwCurrentState != SERVICE_STOPPED) {
                  if (opt_debug) {
                     MessageBox(NULL, _("The Bareos service: " APP_NAME " could not be stopped"),
                        APP_DESC, MB_ICONEXCLAMATION | MB_OK);
                  }
               }
            }
            ...
            if (DeleteService(bareosService)) {
               if (opt_debug) {
                  MessageBox(NULL, _("The Bareos service: " APP_NAME " has been removed"),
                     APP_DESC, MB_ICONINFORMATION | MB_OK);
               }
            } else {
               MessageBox(NULL, _("The Bareos service: " APP_NAME " could not be removed"),
                  APP_DESC, MB_ICONEXCLAMATION | MB_OK);
               stat = 1; /* error */
            }
            CloseServiceHandle(bareosService);
            ...
            if (opt_debug) {
               MessageBox(NULL, _("An existing Bareos service: " APP_NAME " could not be found for "
         ...
         MessageBox(NULL, _("The service Manager could not be contacted - the Bareos service was not removed"),
            APP_DESC, MB_ICONEXCLAMATION | MB_OK);
         return 1; /* error */
      }
      ...
      if (RegOpenKey(HKEY_LOCAL_MACHINE,
         ...
         if (RegDeleteValue(runservices, APP_NAME) != ERROR_SUCCESS) {
            ...
            MessageBox(NULL, _("Could not delete Registry key for " APP_NAME ".\n"
               "The Bareos service could not be removed"), APP_DESC, MB_ICONEXCLAMATION | MB_OK);
         }
         RegCloseKey(runservices);
         return 1;
      }
      ...
      if (opt_debug) {
         MessageBox(NULL, _("The Bareos service has been removed"), APP_DESC, MB_ICONINFORMATION | MB_OK);
      }
   }
   return 0;
}
